    ...
    addData: {
      ...
      get: function () {
        return function (bindHolder, propertyName, propertyValue, dependencies) {
          ...
          // add to propstore
          const propNameDefinition = {}
          if (addsBindings) {
            propNameDefinition.value = this.explicitNonConfigurable(propValue.bind(bindHolder.bindstore))
            propNameDefinition.previousValue = this.explicitNonConfigurable(null)
            propNameDefinition.dependencies = this.explicitNonConfigurable(dependencies)
          } else {
            propNameDefinition.value = this.explicitNonConfigurable(propValue)
            propNameDefinition.previousValue = this.explicitNonConfigurable(propValue)
            propNameDefinition.dependencies = this.explicitNonConfigurable(null)
          }
